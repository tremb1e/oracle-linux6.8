...
static int add_cipher(char *name, struct tse_cipher_elem **current,
		      uint8_t flag)
{
	int i;
	int rc = 0;
	...
	for(i = 0;
		...
		if(strcmp(name, cipher->kernel_name))
			continue;
		(*current)->next = malloc(sizeof(struct tse_cipher_elem));
		if ((*current)->next == NULL)
			return -ENOMEM;
		memset((*current)->next, 0, sizeof(struct tse_cipher_elem));
...
int tse_get_current_kernel_ciphers(
	struct tse_cipher_elem *cipher_list_head)
{
	int rc;

	if (!(rc = tse_get_loaded_ciphers(cipher_list_head)))
		rc = tse_add_crypto_modules(cipher_list_head);
	return rc;
}

/**
 * tse_get_loaded_cipher - creates a linked list of ciphers in
 * the kernel
 ...
 */
int tse_get_loaded_ciphers(struct tse_cipher_elem *cipher_list_head)
{
	...
	struct tse_cipher_elem *current_cipher = cipher_list_head;
	int rc;
	...
	rc = tse_get_proc_mount_point(&proc_mount_point);
	if (rc) {
	...
	if (!(crypto_file = fopen(crypto_full_path, "r"))) {
		syslog(LOG_WARNING, "%s: Failed to open [%s] for reading; This "
		       ...
		       "loaded. If this is not the case, Tse will not be "
...
/**
 * tse_add_crypto_modules - adds cyrpt modules to cipher list
 * @cipher_list_head: the head of the cipher list
 *
 ...
 */
int tse_add_crypto_modules(struct tse_cipher_elem *cipher_list_head)
{
	DIR *dp;
	int rc = 0;
	char *mod_ext;
	...
	struct tse_cipher_elem *current_cipher;
 ...
 **/
int tse_default_cipher(struct tse_cipher_elem **default_cipher,
		 	    struct tse_cipher_elem *cipher_list_head)
{
	struct tse_cipher_elem *current_cipher;
	int i, limit;
...
int tse_free_cipher_list(struct tse_cipher_elem cipher_list_head)
{
	struct tse_cipher_elem *next;
	struct tse_cipher_elem *current = cipher_list_head.next;
...
static struct cipher_name_module_map {
	...
	{"sm4", "sm4.ko", 16, 16, 16, 1, 1},
	{"cipher_null", "cipher_null.ko", 1, 0, 0, 99, 0},
	{NULL, NULL, 0, 0, 0, 0}
};

int tse_get_kernel_ciphers(struct cipher_descriptor *cd_head)
{
...
int tse_get_module_ciphers(struct cipher_descriptor *cd_head)
{
	struct cipher_descriptor *cd_cursor = cd_head;
...
int tse_sort_ciphers(struct cipher_descriptor *cd_head)
{
