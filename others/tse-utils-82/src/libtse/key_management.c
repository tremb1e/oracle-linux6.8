				  ...
				  char *salt, char *src)
{
	...
	for (i=0; i<TSE_MAX_PASSPHRASE_BYTES+1; i++)
			     ...
			     char *wrapping_salt, char *decrypted_passphrase)
{
	...
	if (decrypted_passphrase_bytes > TSE_MAX_PASSPHRASE_BYTES) {
	...
	char *salt)
{
	...
	if (tse_get_version(&version) == 0 &&
	    tse_supports_filename_encryption(version)) {
		if ((rc = tse_add_passphrase_key_to_keyring(
					auth_tok_sig,
