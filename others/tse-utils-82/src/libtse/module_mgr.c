...
static int init_tse_cipher_param_node(uint32_t version)
{
	...
	rc = tse_get_kernel_ciphers(&cd_head);
	if (rc) {
		syslog(LOG_ERR, "%s: Error getting kernel ciphers; rc = [%d]\n",
		       __FUNCTION__, rc);
		goto out;
	}
	rc = tse_get_module_ciphers(&cd_head);
	if (rc) {
						...
						uint32_t version)
{
	...
	rc = init_tse_cipher_param_node(version);
	if (rc) {
		syslog(LOG_ERR,
