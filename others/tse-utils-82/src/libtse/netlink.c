/**
 * Userspace side of netlink communications with Tse kernel
 * module.
 *
			  ...
			  uint16_t msg_flags, uint32_t msg_seq)
{
	...
	if (!nlh) {
		tse_syslog(LOG_ERR, "Failed to allocate memory for "
				"netlink header: %m\n");
		rc = -ENOMEM;
		goto out;
	}
	...
	if (rc < 0) {
		rc = -errno;
		syslog(LOG_ERR, "Failed to send Tse netlink "
		       "message: %m\n");
		goto out;
	}
out:
...
int tse_recv_netlink(struct tse_nl_ctx *nl_ctx,
...
int tse_init_netlink(struct tse_nl_ctx *nl_ctx)
{
	...
	nl_ctx->socket_fd = socket(PF_NETLINK, SOCK_RAW, NETLINK_TSE);
	if (nl_ctx->socket_fd == -1) {
		rc = -errno;
		syslog(LOG_ERR, "Failed to create the Tse "
		       "netlink socket: [%m]\n");
		goto out;
	}
	...
	if (rc) {
		rc = -errno;
		syslog(LOG_ERR, "Failed to bind the Tse netlink "
		       "socket: %m\n");
		goto out;
	}
	syslog(LOG_DEBUG, "Tse netlink socket was successfully "
	       "initialized\n");
out:
	return rc;
}
...
receive:
	rc = tse_recv_netlink(nl_ctx, &emsg, &msg_seq, &msg_type);
	if (rc < 0) {
		syslog(LOG_ERR, "Error while receiving Tse netlink "
		       "message; errno = [%d]; errno msg = [%m]\n", errno);
		error_count++;
		if (error_count > TSE_NETLINK_ERROR_COUNT_THRESHOLD) {
			syslog(LOG_ERR, "Netlink error threshold exceeded "
			       "maximum of [%d]; terminating daemon\n",
			       TSE_NETLINK_ERROR_COUNT_THRESHOLD);
			rc = -EIO;
			goto out;
		}
	} else if (msg_type == TSE_MSG_HELO) {
		syslog(LOG_DEBUG, "Received Tse netlink HELO "
		       "message from the kernel\n");
		error_count = 0;
	} else if (msg_type == TSE_MSG_QUIT) {
		syslog(LOG_DEBUG, "Received Tse netlink QUIT "
		       "message from the kernel\n");
		free(emsg);
		rc = 0;
		goto out;
		...
		rc = tse_send_netlink(nl_ctx, reply,
					   TSE_MSG_RESPONSE, 0,
					   msg_seq);
		if (rc < 0) {
