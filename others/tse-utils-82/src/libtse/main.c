...
int tse_private_is_mounted(char *dev, char *mnt, char *sig, int mounting) {
	...
	char *opt = NULL;
	int mounted;
	if (asprintf(&opt, "tse_sig=%s", sig) < 0) {
		perror("asprintf");
		return 0;
	}
			...
			char *salt, char *passphrase)
{
	...
	if (passphrase_size > TSE_MAX_PASSPHRASE_BYTES) {
		passphrase_sig = NULL;
...
int tse_mount(char *source, char *target, unsigned long flags, char *opts)
{
	...
	if (mount(fullpath_source, fullpath_target, "tse", flags, opts)) {
		rc = -errno;
		syslog(LOG_ERR, "Failed to perform Tse mount: [%m]\n");
		goto out;
	}
