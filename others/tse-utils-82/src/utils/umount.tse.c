...
static void usage()
{
	fprintf(stderr, "\tTse umount helper\n\tusage: "
		"umount [tse mount point]\n"
		);
	exit(-EINVAL);
}
 ...
 * a pointer to the option value.  For example, if name was "tse_sig=",
...
static int unlink_keys_from_keyring(const char *mnt_point)
{
	...
	if (!hasmntopt(mntent, "tse_unlink_sigs")) {
		rc = 0;
		goto end_out;
	}
	rc = get_mount_opt_value(mntent->mnt_opts, "tse_sig=", &fekek_sig);
	if (!rc) {
		fekek_fail = tse_remove_auth_tok_from_keyring(fekek_sig);
	...
	if (!get_mount_opt_value(mntent->mnt_opts,
				 "tse_fnek_sig=", &fnek_sig)
	    && strcmp(fekek_sig, fnek_sig)) {
		fnek_fail = tse_remove_auth_tok_from_keyring(fnek_sig);
		if (fnek_fail == ENOKEY)
			fnek_fail = 0;
		if (fnek_fail) {
