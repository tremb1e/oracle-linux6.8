...
int main(int argc, char *argv[])
{
	...
	if (argc == 2) {
		/* interactive mode */
		passphrase = tse_get_passphrase("Passphrase to wrap");
		if (passphrase)
			wrapping_passphrase =
				tse_get_passphrase("Wrapping passphrase");
	...
	if ((rc = tse_wrap_passphrase(file, wrapping_passphrase, salt,
					   passphrase))) {
		fprintf(stderr, "%s [%d]\n", TSE_ERROR_WRAP, rc);
		fprintf(stderr, "%s\n", TSE_INFO_CHECK_LOG);
		rc = 1;
