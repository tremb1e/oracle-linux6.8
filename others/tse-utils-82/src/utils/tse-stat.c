/**
 * Present statistics on encrypted Tse file attributes
 */
...
int main(int argc, const char *argv[])
{
	...
	int fd = -1;
	ssize_t quant_read;
	struct tse_crypt_stat_user crypt_stat;
	char buf[4096];
	int rc = 0;
	...
	rc = tse_parse_stat(&crypt_stat, buf, quant_read);
	if (rc) {
		printf("Valid Tse metadata information not found in [%s]"
		       "\n", filename);
		rc = 0;
		goto out;
	}
	...
	if (crypt_stat.flags & TSE_ENCRYPTED)
		...
		printf("Plaintext\n");
	if (crypt_stat.flags & TSE_ENABLE_HMAC)
		printf("HMAC enabled\n");
	else
