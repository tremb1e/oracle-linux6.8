...
int main(int argc, char **argv)
{
	...
	char *chrootdir = NULL;
	char *tty = NULL;
	uint32_t channel_type = TSE_DEFAULT_MESSAGING_TYPE;
	...
	while ((long_options_ret = getopt_long(argc, argv, short_options,
		...
		case 'd':
			...
			if (strcmp(optarg, "netlink") == 0)
				channel_type = TSE_MESSAGING_TYPE_NETLINK;
			else if (strcmp(optarg, "miscdev") == 0)
				channel_type = TSE_MESSAGING_TYPE_MISCDEV;
			break;
		case 'h':
		default:
	...
	if (!messaging_type_specified) {
		...
		if (rc) {
			...
			if (version & TSE_VERSIONING_MISCDEV)
				channel_type = TSE_MESSAGING_TYPE_MISCDEV;
			else
				channel_type = TSE_MESSAGING_TYPE_NETLINK;
		}
	...
	pthread_mutex_lock(&mctx_mux);
	rc = tse_init_messaging(&mctx, channel_type);
	if (rc) {
	...
	if (rc) {
		syslog(LOG_ERR, "%s: Error attempting to send message to "
		       "Tse kernel module via transport of type "
