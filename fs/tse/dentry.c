...
static int tse_d_revalidate(struct dentry *dentry, unsigned int flags)
{
	...
	if (d_really_is_positive(dentry)) {
		...
		fsstack_copy_attr_all(inode, tse_inode_to_lower(inode));
		if (!inode->i_nlink)
			return 0;
	}
	return rc;
...
static void tse_dentry_free_rcu(struct rcu_head *head)
{
	kmem_cache_free(tse_dentry_info_cache,
		container_of(head, struct tse_dentry_info, rcu));
}
...
{
	struct tse_dentry_info *p = dentry->d_fsdata;
	if (p) {
		path_put(&p->lower_path);
		call_rcu(&p->rcu, tse_dentry_free_rcu);
	}
}
